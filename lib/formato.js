...
// Ajusta o ANO de uma data ISO (AAAA-MM-DD) para o ano coerente com o mês
// que está sendo visto. Extratos de cartão costumam vir SEM ano (ex.: "15/07"),
// então a IA chuta um ano errado. Aqui mantemos o dia/mês e escolhemos o ano
// (anterior, atual ou seguinte) cujo mês fica mais perto do mês de referência.
// Ex.: vendo "2026-08", uma compra "2024-07-15" vira "2026-07-15".
// Na virada de ano funciona: vendo "2026-01", "12" vira dezembro de 2025.
export function ajustarAnoPorReferencia(dataISO, mesReferencia) {
  if (!dataISO || !mesReferencia) return dataISO;
  const [, mesStr, diaStr] = String(dataISO).slice(0, 10).split("-");
  const mes = Number(mesStr);
  const dia = Number(diaStr);
  if (!mes || !dia) return dataISO;

  const [refAno, refMes] = mesReferencia.split("-").map(Number);
  let melhorAno = refAno;
  let melhorDist = Infinity;
  for (const ano of [refAno - 1, refAno, refAno + 1]) {
    const dist = Math.abs(ano * 12 + mes - (refAno * 12 + refMes));
    if (dist < melhorDist) {
      melhorDist = dist;
      melhorAno = ano;
    }
