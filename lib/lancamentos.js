...
// Atualiza a SÉRIE inteira (recorrente/parcelada): aplica os novos valores
// (tipo, descrição, valor, responsável) a todas as linhas com a MESMA descrição
// ORIGINAL, responsável e forma. NÃO mexe na data (cada mês mantém a sua).
export async function atualizarSerie(original, dados) {
  let q = supabase
    ...
    .eq("forma", original.forma);
  q = original.responsavel_id
    ? q.eq("responsavel_id", original.responsavel_id)
...
// Apaga a SÉRIE inteira de um recorrente/parcelada: todas as linhas com a mesma
// descrição, responsável e forma (ex.: todas as parcelas, ou o recorrente de
// todos os meses). O RLS garante que só apaga o que a pessoa pode.
export async function apagarSerie({ descricao, responsavel_id, forma }) {
  let q = supabase
    .from("lancamentos")
    .delete()
    .eq("descricao", descricao)
    .eq("forma", forma);
  q = responsavel_id ? q.eq("responsavel_id", responsavel_id) : q.is("responsavel_id", null);
  const { error } = await q;
  if (error) throw error;
}
